...
pub const puzzle_state = enum(i16) {
...
fn convertValueToBitMask(val: i16) i16 {
    var mask: i16 = @intFromEnum(cell_values.invalid);

    if (val == 0) {
        mask = 0;
    } else if ((val > 0) and (val <= 9)) {
        mask = std.math.shl(i16, 1, @as(u5, @intCast(val)) - 1);
    }

    return mask;
}

/// Check a sequence of values for repeated values
...
const cell = struct {
    ...
    fn init() @This() {
        var c: cell = .{ .value = @intFromEnum(cell_values.no_value), .cand = @intFromEnum(cell_values.value_mask) };
        return c;
    }

    fn resetCandidate(self: *@This()) void {
        self.cand = @intFromEnum(cell_values.value_mask);
    }

    fn setCandidate(self: *@This(), cand: i16) void {
        self.cand = cand;
    }

    fn getCandidate(self: *const @This()) i16 {
        return self.cand;
    }
};

pub const puzzle = @This();
...
pub fn init() @This() {
    var p = @This(){ .cell = undefined, .row_cand = undefined, .col_cand = undefined, .sub_cand = undefined };
    ...
    while (idx < 9) {
        p.row_cand[idx] = @intFromEnum(cell_values.value_mask);
        p.col_cand[idx] = @intFromEnum(cell_values.value_mask);
        p.sub_cand[idx] = @intFromEnum(cell_values.value_mask);
...
/// Set a Cell value
fn setValue(self: *@This(), row: usize, col: usize, value: i16) void {
    self.cell[row][col].value = convertValueToBitMask(value);

    if (value > 0) {
        self.cell[row][col].setCandidate(@intFromEnum(cell_values.no_value)); // Clears the candidate mask
    } else if (value == 0) {
        self.cell[row][col].setCandidate(@intFromEnum(cell_values.value_mask)); // Resets the candidate mask
    } else {
        self.cell[row][col].setCandidate(@intFromEnum(cell_values.invalid)); // Invalid value
    }
}

/// Get a cell value as integer
fn getValue(self: *const @This(), row: usize, col: usize) i16 {
    return convertBitMaskToValue(self.cell[row][col].value);
}

/// Get the candidate bitmask of a cell
fn getCandidate(self: *const @This(), row: usize, col: usize) u16 {
    return self.cell[row][col].cand;
}

/// Remove a candidate value from a cell
fn removeCandidate(self: *@This(), row: usize, col: usize, cand: u5) void {
...
pub fn import(self: *@This(), input_string: []const u8) void {
    ...
    while ((pos_string < input_string.len) and (pos < 81)) {
        ...
        if ((char_val >= '1') and (char_val <= '9')) {
            self.setValue(row, col, @as(i16, @intCast(char_val - '0')));
        } else {
    ...
    var idx: usize = 0;
    while (idx < 9) {
        self.row_cand[idx] = @intFromEnum(cell_values.value_mask);
        self.col_cand[idx] = @intFromEnum(cell_values.value_mask);
        self.sub_cand[idx] = @intFromEnum(cell_values.value_mask);

        idx += 1;
    }
}

pub fn exportAsString(self: *const @This()) []u8 {
    var output_string: [82]u8 = undefined;
    var pos: usize = 0;

    for (self.cell) |row| {
        for (row) |element| {
            var value = @as(u8, @intCast(convertBitMaskToValue(element.value)));
...
fn checkRows(self: *const @This()) bool {
    var match: bool = true;
...
fn checkCols(self: *const @This()) bool {
    var sequence: [9]i16 = undefined;
    var col: usize = 0;
...
fn checkSubs(self: *const @This()) bool {
    var sub: usize = 0;
...
fn checkEmptyVals(self: *const @This()) puzzle_state {
    var count: usize = 0;

    for (self.cell) |row| {
        for (row) |element| {
            var value = validateBitMask(element.value);
            if (.no_candidates == value) {
                count += 1;
...
fn checkPuzzle(self: *const @This()) !puzzle_state {
...
fn generateRowMasks(self: *@This()) usize {
    var row: usize = 0;
    var count: usize = 0;
    ...
    while (row < 9) {
        ...
        self.row_cand[row] = @intFromEnum(cell_values.value_mask) & ~mask;
...
fn generateColMasks(self: *@This()) usize {
    var col: usize = 0;
    var count: usize = 0;
    ...
    while (col < 9) {
        ...
        self.col_cand[col] = @intFromEnum(cell_values.value_mask) & ~mask;
...
fn generateSubMasks(self: *@This()) usize {
    var count: usize = 0;
    var sub: usize = 0;
    while (sub < 9) {
        var sub_row: usize = 0;
        ...
        self.sub_cand[sub] = @intFromEnum(cell_values.value_mask) & ~mask;
...
fn generateCellMask(self: *@This()) usize {
    var count: usize = 0;
    var row: usize = 0;
    while (row < 9) {
        var col: usize = 0;
...
fn updateCells(self: *@This()) usize {
    var count: usize = 0;
    var row: usize = 0;
    ...
    while (row < 9) {
        ...
        while (col < 9) {
            if (cand_state.one_candidate == validateBitMask(self.cell[row][col].cand)) {
                ...
                self.cell[row][col].cand = @intFromEnum(cell_values.no_value);
                count += 1;
            }
...
fn prunePuzzle(self: *@This()) !puzzle_state {
    var count: usize = 1;
...
pub fn solve(self: *@This()) !puzzle_state {
    var status = try self.prunePuzzle();

    while (status == .not_solved) {
...
test "cell testing" {
    var c = cell.init();
    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.value_mask))), c.cand);

    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.value_mask))), c.getCandidate());

    try std.testing.expectEqual(@as(i16, @intCast(0)), c.value);

    c.value = 1;
    c.cand = 1;
    try std.testing.expectEqual(@as(i16, @intCast(1)), c.value);
    try std.testing.expectEqual(@as(i16, @intCast(1)), c.cand);
}
...
test "get candidates" {
    ...
    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.value_mask))), p.cell[0][0].getCandidate());
}
...
test "remove candidates" {
    ...
    try std.testing.expectEqual(@as(i16, @intCast(0x1FE)), p.cell[0][0].getCandidate());

    p.removeCandidate(0, 0, 2);
    try std.testing.expectEqual(@as(i16, @intCast(0x1FC)), p.cell[0][0].getCandidate());
}
...
test "round trip inport/export string" {
    ...
    for (valid_test_puzzles) |test_string| {
        ...
        try std.testing.expectEqual(@as(usize, @intCast(81)), test_string.len);
    }
}

test "validate masks" {
    try std.testing.expectEqual(cand_state.no_candidates, validateBitMask(0x0));
    try std.testing.expectEqual(cand_state.one_candidate, validateBitMask(@intFromEnum(cell_values.value_1)));
    try std.testing.expectEqual(cand_state.one_candidate, validateBitMask(@intFromEnum(cell_values.value_2)));
    try std.testing.expectEqual(cand_state.one_candidate, validateBitMask(@intFromEnum(cell_values.value_3)));
    try std.testing.expectEqual(cand_state.one_candidate, validateBitMask(@intFromEnum(cell_values.value_4)));
    try std.testing.expectEqual(cand_state.one_candidate, validateBitMask(@intFromEnum(cell_values.value_5)));
    try std.testing.expectEqual(cand_state.one_candidate, validateBitMask(@intFromEnum(cell_values.value_6)));
    try std.testing.expectEqual(cand_state.one_candidate, validateBitMask(@intFromEnum(cell_values.value_7)));
    try std.testing.expectEqual(cand_state.one_candidate, validateBitMask(@intFromEnum(cell_values.value_8)));
    try std.testing.expectEqual(cand_state.one_candidate, validateBitMask(@intFromEnum(cell_values.value_9)));
    try std.testing.expectEqual(cand_state.several_candidates, validateBitMask(0x3));
}

test "convert mask to value" {
    try std.testing.expectEqual(@as(i16, @intCast(1)), convertBitMaskToValue(@intFromEnum(cell_values.value_1)));
    try std.testing.expectEqual(@as(i16, @intCast(2)), convertBitMaskToValue(@intFromEnum(cell_values.value_2)));
    try std.testing.expectEqual(@as(i16, @intCast(3)), convertBitMaskToValue(@intFromEnum(cell_values.value_3)));
    try std.testing.expectEqual(@as(i16, @intCast(4)), convertBitMaskToValue(@intFromEnum(cell_values.value_4)));
    try std.testing.expectEqual(@as(i16, @intCast(5)), convertBitMaskToValue(@intFromEnum(cell_values.value_5)));
    try std.testing.expectEqual(@as(i16, @intCast(6)), convertBitMaskToValue(@intFromEnum(cell_values.value_6)));
    try std.testing.expectEqual(@as(i16, @intCast(7)), convertBitMaskToValue(@intFromEnum(cell_values.value_7)));
    try std.testing.expectEqual(@as(i16, @intCast(8)), convertBitMaskToValue(@intFromEnum(cell_values.value_8)));
    try std.testing.expectEqual(@as(i16, @intCast(9)), convertBitMaskToValue(@intFromEnum(cell_values.value_9)));
    try std.testing.expectEqual(@as(i16, @intCast(0)), convertBitMaskToValue(@intFromEnum(cell_values.no_value)));
    try std.testing.expectEqual(@as(i16, @intCast(-1)), convertBitMaskToValue(@intFromEnum(cell_values.value_mask)));
    try std.testing.expectEqual(@as(i16, @intCast(-1)), convertBitMaskToValue(0x3));
}

test "convert value to mask" {
    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.value_1))), convertValueToBitMask(@as(i16, @intCast(1))));
    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.value_2))), convertValueToBitMask(@as(i16, @intCast(2))));
    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.value_3))), convertValueToBitMask(@as(i16, @intCast(3))));
    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.value_4))), convertValueToBitMask(@as(i16, @intCast(4))));
    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.value_5))), convertValueToBitMask(@as(i16, @intCast(5))));
    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.value_6))), convertValueToBitMask(@as(i16, @intCast(6))));
    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.value_7))), convertValueToBitMask(@as(i16, @intCast(7))));
    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.value_8))), convertValueToBitMask(@as(i16, @intCast(8))));
    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.value_9))), convertValueToBitMask(@as(i16, @intCast(9))));
    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.no_value))), convertValueToBitMask(@as(i16, @intCast(0))));
    try std.testing.expectEqual(@as(i16, @intCast(@intFromEnum(cell_values.invalid))), convertValueToBitMask(@as(i16, @intCast(-1))));
}
...
test "Check sequence" {
    ...
    sequence = [9]i16{ @intFromEnum(cell_values.value_1), @intFromEnum(cell_values.value_2), @intFromEnum(cell_values.value_3), @intFromEnum(cell_values.value_4), @intFromEnum(cell_values.value_5), @intFromEnum(cell_values.value_6), @intFromEnum(cell_values.value_7), @intFromEnum(cell_values.value_8), @intFromEnum(cell_values.value_9) };
    try std.testing.expectEqual(true, checkSequence(&sequence));

    sequence = [9]i16{ @intFromEnum(cell_values.invalid), 0, 0, 0, 0, 0, 0, 0, 0 };
    try std.testing.expectEqual(false, checkSequence(&sequence));
}
...
test "check invalid puzzles" {
    ...
    try std.testing.expectEqual(puzzle_state.not_solved, p.checkPuzzle() catch unreachable);
...
test "generate mask tests" {
    ...
    try std.testing.expectEqual(@as(usize, @intCast(0)), p.generateRowMasks());
    try std.testing.expectEqual(@as(usize, @intCast(0)), p.generateColMasks());
    ...
    // First run
    try std.testing.expectEqual(@as(usize, @intCast(1)), p.generateRowMasks());
    try std.testing.expectEqual(@as(usize, @intCast(2)), p.generateColMasks());
    try std.testing.expectEqual(@as(usize, @intCast(2)), p.generateSubMasks());
    try std.testing.expect(0 < p.generateCellMask());
    // second run
    try std.testing.expectEqual(@as(usize, @intCast(0)), p.generateRowMasks());
    try std.testing.expectEqual(@as(usize, @intCast(0)), p.generateColMasks());
    try std.testing.expectEqual(@as(usize, @intCast(0)), p.generateSubMasks());
    try std.testing.expectEqual(@as(usize, @intCast(0)), p.generateCellMask());
}
...
test "update cells" {
    ...
    p.setValue(0, 8, 9);
    try std.testing.expectEqual(puzzle_state.not_solved, p.checkPuzzle() catch unreachable);
    try std.testing.expectEqual(puzzle_state.not_solved, p.prunePuzzle() catch unreachable);
...
test "Solve simple puzzles" {
    ...
    try std.testing.expectEqual(puzzle_state.solved, try p.prunePuzzle());
    ...
    p.import(valid_test_puzzles[3]);
    try std.testing.expectEqual(puzzle_state.not_solved, try p.prunePuzzle());
}
...
test "Select Candidates" {
    ...
    p.setValue(0, 0, 1);
    try std.testing.expect(puzzle_state.not_solved == try p.prunePuzzle());
    try std.testing.expect(true == p.selectCandidate(&row, &col, &val));
    ...
    p.import(valid_test_puzzles[2]);
    try std.testing.expect(puzzle_state.solved == try p.prunePuzzle());
...
test "solve valid sudoku" {
    ...
    for (valid_test_puzzles) |test_string| {
        ...
        try std.testing.expect(puzzle_state.solved == result);
    }
}
